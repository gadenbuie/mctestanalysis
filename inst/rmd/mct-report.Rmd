...
distractor.pct <- ifelse('distractor.pct' %in% names(report_options),
                         report_options$disctractor.pct, 0.33)
...
Table: Percentage of total respondents ($N=$ `r nrow(mctd$Test.complete)`) from upper (*High*, $N=$ `r distractor.data.counts %>% filter(Group == 'high') %>% .$total`) and lower (*Low*, $N=$ `r distractor.data.counts %>% filter(Group == 'low') %>% .$total`) `r pct_to_text(distractor.pct)` percentiles having chosen each item option.
